...
C_91D1()
{
	...
	for(si = Party.party_size; --si >= 0; ) {
		if(Party.chara[si]._stat == STATUS_GOOD && U4_RND1(7) == 0) {
...
C_9209()
{
	...
	if(CurMode >= MOD_COMBAT) {
		...
		switch(Combat_MAP(Combat._charaY[activeChara], Combat._charaX[activeChara])) {
			case TIL_03: case TIL_44:
				if(Party.chara[activeChara]._stat == STATUS_GOOD) {
					Party.chara[activeChara]._stat = STATUS_POISON;
					C_09D9(activeChara);
					dspl_Stats();
				}
			break;
			case TIL_47:
				if(isCharaConscious(activeChara)) {
					Party.chara[activeChara]._stat = STATUS_SLEEP;
	...
	if(CurMode == MOD_DUNGEON) {
		if((tile_cur & 0xf0) == 0xa0) {
			...
			if(tile_cur == 0x80) {
				u4_puts(/*D_29A9*/U4TEXT_EVT_87);
				Party.f_1dc = 0;
			} else {
				u4_puts((tile_cur < 0x88)?/*D_29B2*/U4TEXT_EVT_90_1:/*D_29C3*/U4TEXT_EVT_90_2);
				C_1584();
			}
		}
		return;
